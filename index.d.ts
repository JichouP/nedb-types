...
export type UpdateOperators<Doc> = {
    ...
    $slice: { [K in keyof Doc]: Doc[K] extends Array<any> ? number : never } &
    Record<DeepKey<Doc>, number>;

    $pop: { [K in keyof Doc]: Doc[K] extends Array<any> ? number : never } &
    Record<DeepKey<Doc>, number>;
...
export default class Nedb<T> extends EventEmitter {
    ...
    ensureIndex(
        ...
        fn?: (err: OptionalError) => void
    ): void;
    removeIndex(
        fieldName: keyof Document<T>,
        fn?: (err: OptionalError) => void
    ): void;
    ...
    updateIndexes(
        updates: Array<{ oldDoc: Document<T>; newDoc: InsertDoc<T> }>
    ): void;
    ...
    insert(
        newDoc: InsertDoc<Document<T>>,
        fn?: (err: OptionalError, document: Document<T>) => void
    ): void;
    insert(
        newDocs: InsertDoc<Document<T>>[],
        fn?: (err: OptionalError, documents: Document<T>[]) => void
    ): void;

    count(
        filter: FilterQuery<Document<T>>,
        fn: (err: OptionalError, n: number) => void
    ): void;
    count(filter: FilterQuery<Document<T>>): CountCursor;

    find(
        filter: FilterQuery<Document<T>>,
        projection?: Projection<Document<T>>
    ): Cursor<T>;
    find(
        filter: FilterQuery<Document<T>>,
        projection: Projection<Document<T>>,
    ...
    findOne(
        filter: FilterQuery<Document<T>>,
        projection?: Projection<Document<T>>,
        fn?: (err: OptionalError, document: Document<T> | null) => void
    ): void;
    findOne(
